...
import { fileAdd } from '@/api/hhl/files/fileAdd'
  ...
  export default{
    ...
    methods:{
      ...
      goCon () { // 返回
        this.$router.replace('/taskFileList/'+this.$route.params.dpId+"/"+this.$route.params.type+"/"+this.$route.params.folderId)
      },
      ...
      handlePreview(file) {//添加讨论图片
        for(let i=0;i<file.data.length;i++){
          this.updateFileData.files.push(file.data[i]);
      ...
      fileAdd(this.updateFileData).then(async res=>{
        this.$message({
          showClose: true,
          message: '恭喜你，上传成功',
          type: 'success'
